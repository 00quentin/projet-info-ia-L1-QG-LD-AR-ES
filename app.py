...
/* ============ PALETTE ============ */
:root {
    --primary:   #1a365d;  /* bleu institutionnel */
    --secondary: #2c5282;
    --accent:    #319795;  /* teal accent */
    --bg:        #f7fafc;
    --card:      #ffffff;
    --border:    #e2e8f0;
    --text:      #2d3748;
...
if "simu_A" not in st.session_state:
    st.session_state.simu_A = None  # dict {df, mc_data, chocs_ia, perf_totale, perf_df, ...}
...
<p style="text-align:center; color:#718096; font-size:0.95em; margin-bottom:28px; letter-spacing:0.5px;">
    Simulation de marché pilotée par IA · Analyse institutionnelle
</p>
""", unsafe_allow_html=True)
...
<div class="qt-card-intro">
    ...
    <p style="font-size: 1.05em; margin-bottom: 0;">
        <strong>Quant Terminal</strong> permet de tester comment un événement économique ou géopolitique — réel ou hypothétique — impacte un portefeuille d'investissement. 
        Une IA interprète votre scénario, un moteur stochastique simule les marchés sur l'horizon choisi, 
        et les métriques de risque institutionnelles vous montrent la solidité de votre stratégie.
    </p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    mode_comparaison = st.toggle(
        ...
        help="Compare l'impact de 2 scénarios différents sur le même portefeuille."
    )
    ...
    if mode_comparaison:
        ...
        st.caption("💡 Astuce : comparez une crise vs une relance, ou deux politiques différentes.")
    else:
        ...
        for i in range(0, len(preset_keys), 2):
            ...
            with col2:
                if i + 1 < len(preset_keys):
                    st.button(
                        preset_keys[i + 1], key=f"preset_{i+1}",
                        on_click=set_event_A,
    ...
    mode_monte_carlo = st.checkbox(
        "Mode Monte-Carlo (50 simulations)",
        value=False,
        help="Lance 50 simulations et affiche la médiane avec bande de confiance à 90%."
    )
...
def lancer_une_simulation(scenario, actifs_sel, duree_j, modele, mc):
    chocs = analyser_evenement_macro(scenario)
    ...
    if mc:
        ...
        df = simuler_marche_dynamique(chocs, jours=duree_j, modele=modele, actifs=actifs_sel)
...
if lancer:
    ...
    scenario_B = st.session_state.event_text_B if mode_comparaison else None

    if len(scenario_A.strip()) < 10 or (mode_comparaison and len(scenario_B.strip()) < 10):
        ...
        msg = "Analyse des scénarios..." if mode_comparaison else "L'Analyste IA étudie votre scénario..."
        if mode_monte_carlo:
            msg = "Monte-Carlo : 50 simulations en cours..."
            if mode_comparaison:
                msg = "Monte-Carlo comparatif : 100 simulations en cours..."
        with st.spinner(msg):
            try:
                result_A, err_A = lancer_une_simulation(scenario_A, actifs_selectionnes, duree, modele_simu, mode_monte_carlo)
                if err_A:
                ...
                if mode_comparaison:
                    result_B, err_B = lancer_une_simulation(scenario_B, actifs_selectionnes, duree, modele_simu, mode_monte_carlo)
                    if err_B:
                ...
                if st.session_state.simu_A:
                    ...
                    for label, res in [("A", st.session_state.simu_A),
                                       ("B", st.session_state.simu_B) if mode_comparaison else (None, None)]:
                        if res is None:
                            continue
                        valeur_finale = 0
                        ...
                        st.session_state.historique_simus.insert(0, {
                            ...
                            "label_compare": label if mode_comparaison else None,
                        })
...
def afficher_dashboard(res, params, titre_prefix=""):
    ...
    st.markdown(f'<div class="qt-section-title">{titre_prefix}Métriques de risque du portefeuille</div>', unsafe_allow_html=True)
    st.caption("Les indicateurs utilisés par les gérants de fonds professionnels pour juger la qualité d'un portefeuille.")

    r1, r2, r3, r4 = st.columns(4)
    r1.metric("Volatilité annualisée", f"{metriques['vol_ann']:.2f} %",
              help="Amplitude des variations. Plus c'est haut, plus le portefeuille bouge.")
    r2.metric("Sharpe Ratio", f"{metriques['sharpe']:.2f}",
              help=">1 = bon | >2 = excellent | <0 = mauvais. Rendement par unité de risque.")
    r3.metric("Max Drawdown", f"-{metriques['max_dd']:.2f} %",
              help="Pire chute depuis un plus-haut. Mesure clé du risque.")
    ...
    fig_bar = px.bar(res["perf_df"], x='Performance (%)', y='Actif', orientation='h',
                     ...
                     text='Performance (%)', title="Performance par actif : les gagnants et les perdants")
...
def afficher_portefeuille(res, params, titre_prefix=""):
    ...
    st.markdown(f"<h3 style='text-align:center; color:#1a365d;'>{titre_prefix}Portefeuille de {cap:,.0f} €</h3>", unsafe_allow_html=True)
...
with tab_dashboard:
    if st.session_state.simu_A is None:
        st.info("Configurez votre scénario à gauche et cliquez sur 'Lancer la simulation' pour démarrer l'analyse.")
    else:
...
if tab_compare is not None:
    with tab_compare:
        if st.session_state.simu_A is None or st.session_state.simu_B is None:
            ...
            st.caption("Même portefeuille, même horizon, deux réalités économiques.")
            ...
            # Graphique de comparaison de la valeur du portefeuille
...
with tab_hist:
    ...
    st.caption("Vos 10 dernières simulations avec leurs performances.")

    if not st.session_state.historique_simus:
        st.info("Aucune simulation dans l'historique. Lancez votre première simulation pour commencer.")
    else:
        ...
        for simu in st.session_state.historique_simus:
            ...
            with st.expander(f"{icone}  {simu['date']}  ·  {simu['profil']}{tag_mc}{label}  ·  {simu['perf']:+.2f}%"):
                ...
                with c3:
                    ...
                    st.markdown(f"<div style='background:{couleur}; color:white; padding:18px; "
                                ...
                                f"<div style='font-size:0.8em; opacity:0.9;'>Gain/Perte net</div>"
                                f"<div style='font-size:1.5em; font-weight:700;'>{gain:+,.0f} €</div>"
                                f"</div>", unsafe_allow_html=True)


# ==========================================
# 13. ACADÉMIE — NIVEAU PROFESSEUR
# ==========================================
...
with tab_academie:
    ...
    with st_outils:
        ...
        st.markdown('<div class="qt-callout">'
                    '<strong>La règle professionnelle :</strong> les prix bruts ne disent rien. '
                    'Seules les <strong>variations relatives</strong> comptent. C\'est pour cela que tous les '
                    'graphiques comparatifs sont affichés en "base 100".'
        ...
        **déplace**. Quand les actions s'effondrent, l'argent va quelque part — typiquement dans 
        ...
        st.markdown('<div class="qt-callout-warn">'
                    '<strong>Piège classique :</strong> regarder uniquement la performance d\'un actif. '
                    'Un +10% sur une action qui a volatilité 40% est bien moins impressionnant qu\'un '
                    '+8% sur une obligation qui a volatilité 5%. La performance doit toujours être '
                    'mise en relation avec le risque pris (voir le Sharpe Ratio section 7).'
                    '</div>', unsafe_allow_html=True)
    ...
    with st_modeles:
        ...
        **LTCM** en 1998 — deux prix Nobel au conseil, et pourtant ils ont fait faillite parce qu'ils 
        ...
        st.markdown('<div class="qt-callout-warn">'
                    '<strong>Leçon retenue :</strong> en finance, les événements "impossibles" arrivent tous les 10 ans. '
    ...
    with st_cas:
        ...
        **Enseignement clé :** en 2020, on a appris qu'il y a désormais un **"put implicite" des banques 
        ...
        st.markdown('<div class="qt-callout">'
                    ...
                    '<li>Les krachs arrivent tous les 8-12 ans en moyenne. C\'est un <strong>fait statistique</strong>.</li>'
                    '<li>Chaque fois, les investisseurs disent "cette fois c\'est différent". Ce ne l\'est jamais vraiment.</li>'
                    '<li>Le meilleur moment pour acheter est souvent le plus inconfortable (mars 2009, mars 2020).</li>'
                    '<li>Diversifier à travers des actifs décorrélés (actions, obligations, or, cash) reste la seule défense sérieuse.</li>'
                    '<li>Les liquidités disponibles (cash) sont aussi une position : elles permettent d\'acheter quand tout est bradé.</li>'
    ...
    with st_strat:
        ...
        **Qui l'utilise ?** Les hedge funds systématiques comme **AHL (Man Group)**, **Winton Capital**, 
        ...
        st.markdown('<div class="qt-callout">'
                    'Il n\'y a pas de "meilleure" stratégie dans l\'absolu. Tout dépend de :'
                    '<ul style="margin:8px 0 0 0; padding-left:20px;">'
                    '<li><strong>Votre horizon</strong> : 5 ans vs 30 ans, ce n\'est pas la même chose.</li>'
                    '<li><strong>Votre tolérance au risque</strong> : pouvez-vous dormir si votre portefeuille perd -40% ?</li>'
                    '<li><strong>Vos objectifs</strong> : préservation du capital, rendement, héritage ?</li>'
                    '<li><strong>Vos compétences</strong> : le value investing demande de savoir analyser un bilan, '
                    'le momentum d\'avoir une discipline de fer.</li>'
                    '</ul></div>', unsafe_allow_html=True)
    ...
    with st_lex:
        ...
        - **Gamma (Γ)** — Sensibilité du delta au prix du sous-jacent. Important pour les gros portefeuilles d'options.
    ...
    with st_methodo:
        ...
        **Attention** : la VaR ne dit rien sur ce qui se passe dans les 5% restants — les *tail risks*. 
        ...
        st.markdown('<div class="qt-callout-warn">'
                    '<strong>Pour l\'honnêteté intellectuelle</strong>, voici les limites de Quant Terminal :'
                    '<ul style="margin:8px 0 0 0; padding-left:20px;">'
                    '<li>Les volatilités sont supposées <strong>constantes</strong>, alors qu\'en réalité elles '
                    'se regroupent en clusters (effet GARCH).</li>'
                    '<li>Les actifs sont traités <strong>indépendamment</strong>, alors qu\'en crise toutes '
                    'les corrélations montent à 1 (voir 2008).</li>'
                    '<li>L\'IA fournit une estimation <strong>globale</strong> du choc, pas une trajectoire '
                    'fine dans le temps.</li>'
                    '<li>Les distributions restent <strong>gaussiennes</strong>, sauf dans le mode Historique. '
                    'La réalité a des queues encore plus épaisses.</li>'
                    '<li>Pas de prise en compte des <strong>coûts de transaction</strong>, des <strong>spreads bid-ask</strong>, '
                    'ni de la <strong>liquidité</strong>.</li>'
                    '</ul>'
                    '<p style="margin-top:10px; margin-bottom:0;">Quant Terminal est un outil <strong>pédagogique</strong>. '
                    'Pour un usage professionnel réel, il faudrait ajouter ces raffinements et calibrer '
                    'le modèle sur des données de marché en temps réel.</p>'
                    '</div>', unsafe_allow_html=True)
